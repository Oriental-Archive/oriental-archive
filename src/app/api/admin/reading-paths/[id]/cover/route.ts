...
export async function POST(
  ...
  try {
    ...
    const readingPath = await prisma.readingPath.findUnique({ where: { id } });
    ...
    const previousKey = readingPath.coverImageStorageKey;
    await prisma.readingPath.update({ where: { id }, data: { coverImageStorageKey: storageKey } });
    ...
    await writeAuditLog({
      ...
      action: "reading_path.update_cover",
